...
- generate-blocker-ical - Code for the application's Lambda function.
- events - Invocation events that you can use to invoke the function.
- generate-blocker-ical/tests - Unit tests for the application code. 
...
```bash
hof-solterbeck-ical$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
hof-solterbeck-ical$ sam local start-api
hof-solterbeck-ical$ curl http://localhost:3000/
```
...
```bash
hof-solterbeck-ical$ sam logs -n HelloWorldFunction --stack-name hof-solterbeck-ical --tail
```
...
```bash
hof-solterbeck-ical$ cd hello-world
...
```bash
aws cloudformation delete-stack --stack-name hof-solterbeck-ical
```